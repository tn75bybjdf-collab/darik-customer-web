...
export default function StorefrontPreviewModal({ open, retailerId, form, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from("products").select("id,direct_name,direct_name_ar,name,direct_price,app_price,direct_pricing_mode,direct_photo_url,official_product_thumbnail_url,official_product_photo_url,retailer_raw_photo_url,direct_store_category_id,direct_product_status,storefront_visible,created_at").eq("retailer_id", retailerId).neq("direct_product_status", "archived").order("storefront_sort_order").limit(100),
