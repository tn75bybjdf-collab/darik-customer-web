...
export default function DarikDirectProductsPage() {
  ...
  useEffect(() => {
    ...
    loadCatalog();
  }, [catalogAccessAllowed193, catalogContextFallback193, catalogUnlocked190, loadCatalog, router, selectedRetailerId, selectedStore]);
  ...
  async function saveProduct(event: FormEvent) {
    ...
    if (isFurnitureColorVariants216) {
      ...
      if (furnitureColorResult216.error) {
        setSaving(false);
        setError(
          `The product was saved, but its color/shade options could not be saved. / تم حفظ المنتج، لكن تعذر حفظ خيارات الألوان / الدرجات. ${furnitureColorResult216.error.message}`
        );
        await loadCatalog();
        return;
      }
